...
(() => {
    ...
    describe('Testing collections block', () => {
        function getRaw() {
            return JSON.parse(fs.readFileSync(__dirname + '/fixtures/collections.json', 'utf8'));
        }
        ...
        it('tests', () => {
            ...
            data = helpers.convert(getRaw());
            data = helpers.filter(data, (item, category, prefix) => prefix.split('-').shift() === 'fa');
            expect(helpers.prefixes(data)).to.be.eql(['fa-solid', 'fa-regular', 'fa', 'fa-brands']);
