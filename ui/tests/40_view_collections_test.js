...
(() => {
    ...
    describe('Testing collections view', () => {
        const generalPrefixes = ['mdi', 'mdi-light', 'ic', 'uil', 'jam', 'ion', 'fa-solid', 'fa-regular', 'vaadin', 'icomoon-free', 'dashicons', 'entypo', 'flat-color-icons', 'ant-design', 'feather', 'fe', 'gridicons', 'ps', 'el', 'foundation', 'typcn', 'subway', 'raphael', 'icons8', 'wpf', 'iwwa', 'topcoat', 'ei', 'bytesize', 'maki', 'oi', 'octicon', 'et', 'fa', 'zmdi', 'whh', 'si-glyph', 'ls', 'simple-line-icons', 'flat-ui', 'vs', 'websymbol', 'il', 'bpmn', 'fontelico'];
        const emojiPrefixes = ['noto', 'noto-v1', 'twemoji', 'emojione', 'emojione-monotone', 'emojione-v1', 'fxemoji'];
        ...
        it('basic list', done => {
            ...
            events.subscribe('view-loaded', view => {
                ...
                expect(Object.keys(blocks.collections.collections)).to.be.eql(['General', 'Emoji', 'Thematic']);
                expect(Object.keys(blocks.collections.collections.General)).to.be.eql(generalPrefixes);
                expect(Object.keys(blocks.collections.collections.Emoji)).to.be.eql(emojiPrefixes);
                ...
                expect(blocks.collections.empty()).to.be.equal(false);
                expect(blocks.collections.showCategories).to.be.equal(true);
                expect(Object.keys(blocks.collections.collections)).to.be.eql(['General', 'Thematic']);
                expect(Object.keys(blocks.collections.collections.General)).to.be.eql(['fa-solid', 'fa-regular', 'fa']);
                ...
                expect(blocks.collections.empty()).to.be.equal(false);
                expect(blocks.collections.showCategories).to.be.equal(true);
                expect(Object.keys(blocks.collections.collections)).to.be.eql(['General']);
                expect(Object.keys(blocks.collections.collections.General)).to.be.eql(['mdi', 'mdi-light', 'zmdi']);
        ...
        it('one category', done => {
            ...
            events.subscribe('view-loaded', view => {
                ...
                expect(blocks.collections.empty()).to.be.equal(false);
                expect(blocks.collections.showCategories).to.be.equal(true);
                expect(Object.keys(blocks.collections.collections)).to.be.eql(['Emoji']);
                expect(Object.keys(blocks.collections.collections.Emoji)).to.be.eql(emojiPrefixes);
            ...
            // Create view
            viewItem = collectionsView(instance, {
                category: 'Emoji'
            }, null);
            expect(viewItem.loading).to.be.equal(true);
        });
        ...
        it('from route', done => {
            ...
            events.subscribe('view-loaded', view => {
                ...
                expect(blocks.collections.empty()).to.be.equal(false);
                expect(blocks.collections.showCategories).to.be.equal(true);
                expect(Object.keys(blocks.collections.collections)).to.be.eql(['Thematic']);
        ...
        it('loading collections data', done => {
            ...
            let check = () => {
                if (triggered['view-loaded'] && triggered['collections-loaded']) {
                    ...
                    expect(api.log).to.be.eql([base + 'collections?version=' + api._version]);
                    done();
                }
            };
