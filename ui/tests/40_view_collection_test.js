    ...
    describe('Testing collection view', () => {
        ...
        it('basic list', done => {
            ...
            events.subscribe('view-loaded', view => {
                ...
                expect(blocks.icons.icons.filter(icon => icon.name === 'window-minimize').length).to.be.equal(1);
                ...
                expect(blocks.icons.icons.filter(icon => icon.name === first).length).to.be.equal(0);

                done();
            });

            // Create view
            viewItem = collectionView(instance, {
                prefix: 'fa-regular',
                page: 3 // last page
            }, null);
        ...
        it('filter uncategorized', done => {
            ...
            // Set API data
            api.setFakeData('collection', apiParams('mdi'), JSON.parse(fs.readFileSync(__dirname + '/fixtures/mdi.json', 'utf8')), 20);
        ...
        it('filter by suffix', done => {
            ...
            events.subscribe('view-loaded', view => {
                ...
                // Check for usb-*
                expect(blocks.icons.icons.filter(icon => icon.name.split('-').shift() === 'usb')).to.be.eql([{
                    prefix: 'ant-design',
                    name: 'usb-outline',
                    themeSuffix: 'Outline'
                }]);
        ...
        it('filter by keyword', done => {
            ...
            // Set API data
            api.setFakeData('collection', apiParams('fa'), JSON.parse(fs.readFileSync(__dirname + '/fixtures/fa.json', 'utf8')), 20);
        ...
        it('test mix of actions', done => {
            ...
            events.subscribe('view-loaded', view => {
                ...
                // Reset suffix
                expect(blocks.themeSuffixes.action(blocks.themeSuffixes.active[0])).to.be.equal(view);
                blocks = view.render();
