...
class Container extends Component {
	...
	_loadIconify() {
		...
		this.iconify = core.create(
			Object.assign(defaultParams, params.iconify ? params.iconify : {}, {
				autoload: autoload,
				ui_events: {
					// Iconify views
	...
	resetPlugin() {
		...
		this.route.iconify =
			typeof params.prefix !== 'string' || !params.prefix.length
				? {
						type: 'collections',
				  }
				: {
	...
	_onDeleteNotice(time) {
		...
		this.notices = this.notices.filter(
			notice => notice.expires > currentTime && notice.time !== time
		);
