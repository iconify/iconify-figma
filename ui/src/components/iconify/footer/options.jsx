...
class FooterOptions extends Component {
	...
	render() {
		return (
			<FooterBlock type="options" title={lang.customize}>
	...
	renderColorPicker() {
		...
		let color =
				typeof props.transformations.color === 'string'
					? props.transformations.color
					: '',
			active = color !== '';

		return (
			<OptionsPanel type="color" title={lang.color} active={active}>
				<ColorInput
					className={active ? 'plugin-input--outlined' : ''}
					placeholder="#000"
					defaultColor="#000"
					value={color}
					onChange={this._onColorChange.bind(this)}
					onTemporaryChange={this._onColorChange.bind(this)}
				/>
			</OptionsPanel>
		);
	}
	...
	renderHeight() {
		...
		return (
			<OptionsPanel type="height" title={lang.height} active={active}>
				<Input
					icon="height"
					className={
						'plugin-input--number' + (active ? ' plugin-input--outlined' : '')
					}
					placeholder={defaultHeight}
					value={height}
					onChange={this._onHeightChange.bind(this, true)}
					onTemporaryChange={this._onHeightChange.bind(this, false)}
				/>
			</OptionsPanel>
		);
	}
	...
	renderRotation() {
		...
		return (
			<OptionsPanel type="rotation" title={lang.rotate} active={active}>
				<Select
					icon="angle"
					className={
						'plugin-input--angle' + (active ? ' plugin-input--outlined' : '')
					}
					placeholder={rotationOptions[0]}
					value={value}
					options={rotationOptions}
					onChange={this._onRotationChange.bind(this)}
				/>
			</OptionsPanel>
		);
	}
	...
	renderFlip() {
		...
		return (
			<OptionsPanel type="flip" title={lang.flip} active={hFlip || vFlip}>
				<IconButton
					icon="arrows-horizontal"
					title={lang.hFlip}
					onClick={this._onFlipClick.bind(this, 'hFlip')}
					active={hFlip}
				/>
				<IconButton
					icon="arrows-vertical"
					title={lang.vFlip}
					onClick={this._onFlipClick.bind(this, 'vFlip')}
					active={vFlip}
				/>
			</OptionsPanel>
		);
