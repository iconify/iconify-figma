...
function HTMLCodeBlock(props) {
    ...
    if (icon.hFlip) {
        attribs.push(' data-flip="horizontal' + (icon.vFlip ? ',vertical' : '') + '"');
    } else if (icon.vFlip) {
    ...
    // Generate HTML code
    let html = '<iconify-icon data-icon="' + name + '"' + attribs.join('') + '></iconify-icon>',
        script = '<script src="https://code.iconify.design/' + majorVersion + '/' + version + '/iconify.min.js"></script>';

    // Extra code for footer
    let footerCode = null;
    if (props.footer) {
        let html2 = '<iconify-icon data-icon="' + name + '" style="font-size: 24px;' + (icon.hasColor ? ' color: red;' : '') + '"></iconify-icon>';

        footerCode = [
            props.hasColor ? lang.htmlCodeTextColor : lang.htmlCodeText,
            // <div key="footer2" className="plugin-code-sample">{html2}</div>,
            <div key="footer3" className="plugin-code-sample">{html2.replace('<iconify-icon', '<span class="iconify" data-inline="false"').replace('</iconify-icon>', '</span>')}</div>
        ];
    }

    return <div className="plugin-code plugin-code--html">
        {lang.htmlCode1}
