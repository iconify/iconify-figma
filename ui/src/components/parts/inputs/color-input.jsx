...
class ColorInput extends Input {
    constructor(props) {
        ...
        let defaultColor = typeof props.defaultColor === 'string' && props.defaultColor.length ? props.defaultColor : '#000';
    ...
    _getProps() {
        return Object.assign({}, this.props, {
            className: 'plugin-input--color' + (this.props.className ? ' ' + this.props.className : ''),
            icon: 'color',
            spellcheck: false,
     ...
     */
    _renderIcon(name) {
        return <span className="plugin-input-color-sample" style={{backgroundColor: this.state.sampleColor}} />;
    }
    ...
    _onTemporaryColorChange(value) {
        ...
        if (value === '') {
            if (typeof props.onTemporaryChange === 'function' && state.lastTemporaryChange !== value) {
        ...
        if (fixedValue !== '') {
            if (typeof props.onTemporaryChange === 'function' && state.lastTemporaryChange !== fixedValue) {
    ...
    _onColorChange(value) {
        ...
        if (value.length) {
            value = this._validateColor(value, state.defaultSampleColor === state.sampleColor ? '' : state.sampleColor);
        }
