...
class AlignInput extends Component {
	render() {
		...
		['x', 'y'].forEach(side => {
			...
			constants[langKey].forEach(key => {
				options[side][key] =
					phrases[langKey] !== void 0 && phrases[langKey][key] !== void 0
						? phrases[langKey][key]
		...
		return (
			<div className="plugin-align">
				<div className="plugin-align-box">
					<a
						href="#"
						onClick={this._onBoxClick.bind(this)}
						dangerouslySetInnerHTML={{ __html: this._renderSVG([x, y]) }}
				...
				<div className="plugin-align-select">
					...
					<Select
						...
						value={y}
						options={options.y}
						onChange={this._onChange.bind(this, false)}
					/>
				</div>
			</div>
		);
	...
	_renderSVG(values) {
		...
		Object.keys(iconParts).forEach(key => {
			...
			paths[selected ? 'selected' : 'unselected'].push(
				'<path d="' +
					iconParts[key][selected ? 'selected' : 'unselected'] +
					'" class="plugin-box-icon--' +
		...
		return (
			result + paths.unselected.join('') + paths.selected.join('') + '</svg>'
		);
	}
	...
	_onBoxClick(event) {
		...
		if (x === y || revX === y) {
			...
			if (xCentered !== yCentered) {
				...
				this._onChange(
					yCentered,
					constants[yCentered ? 'alignX' : 'alignY'][1]
				);
				return;
			}
		}
