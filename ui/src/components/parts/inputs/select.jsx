...
class Select extends Component {
	...
	render() {
		let props = this.props,
			state = this.state,
			className =
				'plugin-input plugin-input--select' +
				(typeof props.className === 'string' ? ' ' + props.className : ''),
			wrapperClassName =
				'plugin-select-container plugin-input-wrapper plugin-input-wrapper--select' +
				(typeof props.wrapperClassName === 'string'
					? ' ' + props.wrapperClassName
					: ''),
			icon = null,
			value =
				props.value === void 0 || props.options[props.value] === void 0
					? ''
		...
		return (
			<div className={wrapperClassName} ref={node => (this.wrapper = node)}>
				<input {...attribs} ref={input => (this.input = input)} />
				{icon}
				<span className="plugin-input-dropdown">
