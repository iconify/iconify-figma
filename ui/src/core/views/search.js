...
module.exports = (instance, params, parent) => {
	...
	view.getCollectionFilters = () => {
		...
		Object.keys(view._data.collections).forEach(prefix => {
			result[prefix] = view._data.collections[prefix].title
				? view._data.collections[prefix].title
				: prefix;
		});
		...
		switch (key) {
			...
			case 'collections':
				...
				childView = view.createChild('collection', {
					prefix: value,
					search:
						typeof optional === 'string' ? optional : view.route.params.keyword,
				});
				return childView;
		}
    ...
    */
	...
	if (view._defaultPrefixes) {
		queryParams.prefixes =
			typeof view._defaultPrefixes === 'string'
				? view._defaultPrefixes
				: view._defaultPrefixes.join(',');
	}
	...
	api.load('search', queryParams, data => {
		...
		if (
			!view.route.params.full &&
			view._data.icons.length >= queryParams.limit
		) {
			view.more = true;
		}
