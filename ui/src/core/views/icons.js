...
module.exports = view => {
    ...
    view.searchIcons = (icons, customSearches) => {
        ...
        if (view.route.params.search !== '') {
            let keywords = view.route.params.search.toLowerCase().split(' ').map(keyword => keyword.trim()).filter(keyword => keyword.length > 0);
    ...
    view.iconsAction = (key, value, optional) => {
        switch (key) {
            ...
            case 'collections':
                // Collections filter
                if (view.parent && view.blocks.collections.active.indexOf(value) !== -1) {
